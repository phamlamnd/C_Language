...
Output : largest subarray sum is { 4, -1, -2, 1, 5 } = 7*/
...
int main()
{
	...
	int n = 8;
	printf("Largest sum: %d", largestSum(arr, 8));
	
	//Exercise 3:
	/*int arr[] = {1, 2, 4, 6, 3, 7, 8};
	int n = 7;
	printf("Missing number: %d", findMissingNumber(arr, 7));*/
	
	//Exercise 4:
	/*int arr[] = {4, 5, 1, 2};
	int n = 4;
	reverseArray(arr, 4);
	recursiveArray(arr, 4, 0);
	int i;
	for(i = 0; i < n; i++)
	{
		printf("%d ", arr[i]);
	...
	int arr3[MAX];
	int m = 5, n = 4, k = 0;
	//unionArray(arr1, arr2, arr3, m, n, &k);
	intersectionArray(arr1, arr2, arr3, m, n, &k);
	int i;
	for(i = 0; i < k; i++)
	{
		printf("%d ", arr3[i]);
